...
```{r}
SpatialFeaturePlot(all_samples$skin235, features = 'nFeature_Spatial')
```


```{r}
SpatialFeaturePlot(
  all_samples$skin235, features = c("KRT10", 'COL1A1'), image.alpha = 0.1) 
```
```{r}
...
p <- all_samples %>% map(function(i) {
  ...
  complexity <- i$nFeature_Spatial %>% median()
  SpatialFeaturePlot(i, features = c("COL1A1"), image.alpha = 0.1) + 
    labs(title = i$id %>% unique(), 
         subtitle = paste0('Median genes: ', complexity)) +
    theme(plot.background = element_rect(color = 'black'))
}) %>% wrap_plots(ncol = 4)
