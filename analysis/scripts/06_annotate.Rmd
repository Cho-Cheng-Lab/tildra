...
```{r}
 sankey_input <- prior_annotations %>% 
  select(`Current Cluster` = seurat_clusters,
         `Old Cluster` = label) %>% 
  make_long(`Current Cluster`, `Old Cluster`)

p <- ggplot(sankey_input, 
                aes(x = x, 
                    next_x = next_x, 
                    node = factor(node, levels = c(0:42, as.character(prior_annotations$label %>% unique() %>% sort()))),
                    next_node = next_node,
                    fill = factor(node, levels = c(0:42, as.character(prior_annotations$label %>% unique() %>% sort()))),
                    label = node)) +
...
#mclapply(merged_annotations$seurat_clusters %>% unique() %>% sort(), mc.cores = 40, function(which_cluster) {
  ...
  select(`Current Cluster` = seurat_clusters,
         `RashX Cluster` = cluster) %>% 
  make_long(`Current Cluster`, `RashX Cluster`)
...
#mclapply(merged_annotations$cluster %>% unique() %>% sort(), mc.cores = 40, function(which_cluster) {
  ...
  sankey_input <-  cluster_subset %>% 
    ...
    select(`Current Cluster` = seurat_clusters,
           `RashX Cluster` = cluster) %>% 
    make_long(`RashX Cluster`, `Current Cluster`) %>% 
