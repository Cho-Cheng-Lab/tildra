...
```
#### Leave one out stats only
```{r}

which_cluster <- 'Trm'
df <- map(patients, function(i) {
  
  genes_all <- il23_group_response %>% filter(comparison == i) %>% pull(gene) %>% unique()
  
  res <- map(pseudo$patient %>% unique() %>% sort() %>% as.character(), function(which_patient) {
    
    df <- plot_radar_patient(pseudo = pseudo,
                       cluster = which_cluster,
                       patient = which_patient,
                       genes = genes_all,
                       scale = 'within',
...
```{r}
stats_summary <- df %>% 
  group_by(leave_out, patient, p) %>% 
  summarize(mean = mean(Mid),
            median = median(Mid)) %>% 
  mutate(significant = ifelse(p < 0.05, ' Significant', 'Non-significant'))

stats_summary %>% write_tsv(here(output_dir, 'leave_out_summary_stats.tsv'))
stats_summary
```
```{r}
p <- stats_summary %>% 
  mutate(label = paste('Leave out', leave_out)) %>% 
...
ggsave(plot = p,
             ...
             filename = paste0('leave_out_summary', '.pdf'))
