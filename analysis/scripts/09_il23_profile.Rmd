...
### Directories
```{r, message=FALSE}
output_dir <- here('analysis/output/09_il23_profile') # analysis file output directory
...
plot_radar_patient <- function(pseudo,
  ...
  if(test) {
    ...
    subtitle <- paste0('t p = ', round(res$t$p.value, 3), ', Wilcoxon p = ', round(res$wilcox$p.value, 3))
...
pseudo <- pseudobulk(seuratobj,
                     ...
                     genes = c(genes_core, il23_genes_all %>% unlist()) %>% unique(),
...
p <- plot_radar_patient(pseudo = pseudo,
                   cluster = 'Mac1',
                   patient = 'P1',
                   genes = c(genes_core, il23_genes_strict$Mac1),
                   scale = 'across')
