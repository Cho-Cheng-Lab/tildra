...
```{r}
outlier <- RunPresto(lymphocytes, ident.1 = 20) %>% rownames_to_column('gene') %>% mutate(pct.diff = pct.1 - pct.2) %>% arrange(-pct.diff)
outlier
```

### Extract T lymphocytes and recluster
```{r}
tcell_clusters <- lymphocytes$seurat_clusters %>% setdiff(20)
tcells <- subset(lymphocytes, subset = (seurat_clusters %in% tcell_clusters)) %>% DietSeurat() %>% FindVariableFeatures(nfeatures = 3000) %>% ScaleData() %>% RunPCA() %>% RunHarmony(batch) 

ElbowPlot(tcells, ndims = 50, reduction = 'harmony')
```
```{r}
tcells <- tcells %>% recluster(dims = dims)

p <- seurat_feature(tcells, facet_hide = TRUE, title = 'T Cell Subcluster', legend_position = 'none')
...
```{r}
tcell_cluster_sample_tally <- tcells@meta.data %>% 
  select(seurat_clusters, id) %>% 
  unique() %>% 
  add_count(seurat_clusters) %>% 
  arrange(n)

tcell_cluster_sample_tally
```
```{r}
clusters_to_remove <- tcell_cluster_sample_tally %>% filter(n == 1) %>% pull(seurat_clusters) %>% as.character()
clusters_to_remove

tcells_to_remove <- tcells@meta.data %>% filter(seurat_clusters %in% clusters_to_remove) %>% rownames()
```
