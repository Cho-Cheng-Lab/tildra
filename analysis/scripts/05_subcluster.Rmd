---
title: "Subclustering"
author: "David Wu"
---

### Use the here package for robust relative paths
```{r}
library(here)
```

### Optionally convert this Rmd to R script 
```{r eval=FALSE, message=FALSE}
current_file <- rstudioapi::getActiveDocumentContext()$path
output_file <- stringr::str_replace(current_file, '.Rmd', '.R')
knitr::purl(current_file, output = output_file)
file.edit(output_file)
```

### Directories
```{r, message=FALSE}
output_dir <- 'analysis/output/05_subcluster' # analysis file output directory
data_dir <- 'data/derived/tildra' # data file output directory

dir.create(output_dir, showWarnings = FALSE)
dir.create(data_dir, showWarnings = FALSE)
```

### Loading the libraries
```{r, message=FALSE}
library(Seurat) 
library(tidyverse)
library(ggExtra)
library(SeuratWrappers)
library(wutilities) # devtools::install_github('symbiologist/wutilities')
library(tictoc)
library(harmony)
```
### Functions

```{r}
recluster <- function(seuratobj, 
                      assay = 'RNA',
                      dims = 1:20,
                      resolution = c(0.2, 1.0, 2.0),
                      default_ident = 'RNA_snn_res.1') {
  
    ### RunUMAP, FindNeighbors, and FindClusters 
  
  seuratobj <- seuratobj %>% 
    RunUMAP(reduction = "harmony", dims = dims, assay = assay) %>% 
    FindNeighbors(reduction = "harmony", dims = dims, assay = assay) %>% 
    FindClusters(resolution = resolution)
  
  ### Fix cluster factor levels
  clusterings <- colnames(seuratobj@meta.data) %>% str_subset('_res')
  
  for(i in clusterings) {
    clusters <- seuratobj@meta.data[[i]]
    seuratobj[[i]] <- factor(clusters, levels = levels(clusters) %>% as.numeric() %>% sort())
  }
  
  Idents(seuratobj) <- default_ident
  seuratobj$seurat_clusters <- seuratobj[[default_ident]]
  
  seuratobj
}
```
```{r}
seuratobj <- read_rds(file.path(data_dir, 'seuratobj_clustered.rds'))
seuratobj
```

### Visualize
```{r}
p <- seurat_feature(seuratobj, features = c('RNA_snn_res.0.2'), facet_hide = TRUE, legend_position = 'none', title = 'Resolution: 0.2')

p
```

```{r}
seurat_feature(seuratobj, feature = 'RNA_snn_res.0.2', facets = 'RNA_snn_res.0.2', label = FALSE)
```
### Reynolds et al markers

```{r}
markers_reynolds <- c('KRT14', 'KRT5', 
                      'GATA3', 'KRTDAP',
                      'CRYAB', 'TYRP1',
                      'DKK3', 'PLEKHA4',
                      'LUM', 'APOD',
                      'PHLDA2', 'CCL2',
                      'IL6', 'PDGFRA',
                      'COL1A1', 'CXCL12',
                      'CD82', 'CCL19', 'KCNE4', 'CPE', 'TAGLN', 'MYL9', 'HEY1', 'CCL14', 'PECAM1',
                      'ACKR1', 'SELE', 'SNCG', 'HES1', 'MMRN1', 'XCL1' ,'XCL2', 'KLRC1', 'CCL5', 'GZMA', 
                      'CD8A', 'CD8B', 'IL7R', 'FOXP3', 'TIGIT', 'TPSB2', 'TPSAB1', 'CD79A', 'JCHAIN', 'CD163',
                      'C1QB', 'FCGR2A', 'MS4A6A', 'IL23A', 'CLEC9A', 'CLEC10A', 'CD1C', 'CD207', 'CD1A', 
                      'CD14', 'IL1B', 'CCR7', 'IDO1')


```

```{r fig.height=10, fig.width=10}
Idents(seuratobj) <- 'RNA_snn_res.0.2'
p <- DotPlot(seuratobj, features = rev(markers_reynolds), cluster.idents = FALSE) + coord_flip()
p
```




### Keratinocytes
```{r fig.height = 3, fig.width = 12}
markers_keratinocytes <- c('KRT5', 'KRT14', 'GATA3', 'KRTDAP')
seurat_feature(seuratobj, features = markers_keratinocytes, nrow = 1)
```


### Major cell types
```{r}
seurat_feature(seuratobj, 
               features = c('CD3D', #lymphocytes
                            'IGKC', # b cells
                            'TPSAB1', # mast cells
                            'COL6A2')) # endothelial cells
```
```{r}
seurat_feature(seuratobj, features = 'RNA_snn_res.2')
```

### Subclusters
Use UMAP as input to clustering for cleanly selecting clusters
```{r}
seuratobj <- seuratobj %>% 
  FindNeighbors(reduction = 'umap', 
                dims = 1:2) %>% 
  FindClusters(resolution = 0.01)

seuratobj %>% seurat_feature(features = 'seurat_clusters')
```
### Subscluster and re-harmonize
```{r}
apc_clusters <- 1
lymphocyte_clusters <- c(0, 2, 3)

batch <- c('treatment', 'patient', 'flow', 'mouse', 'chemistry')

apcs <- subset(seuratobj, subset = seurat_clusters %in% apc_clusters) %>% FindVariableFeatures(nfeatures = 3000) %>% ScaleData() %>% RunPCA() %>% RunHarmony(batch)
lymphocytes <- subset(seuratobj, subset = seurat_clusters %in% lymphocyte_clusters) %>% FindVariableFeatures(nfeatures = 3000) %>% ScaleData() %>% RunPCA() %>% RunHarmony(batch)

p1 <- seurat_feature(apcs, facet_hide = TRUE)
p2 <- seurat_feature(lymphocytes, facet_hide = TRUE)
p1 + p2
```
```{r}
ElbowPlot(apcs, ndims = 50, reduction = 'harmony')
```

```{r}
ElbowPlot(lymphocytes, ndims = 50, reduction = 'harmony')
```

### Cluster subgroups separately, then recombine objects for grouped analysis
```{r}
dims <- 1:20
apcs <- apcs %>% recluster(dims = dims)
```
```{r}
p <- seurat_feature(apcs, facet_hide = TRUE)
p
```

```{r}
apcs@meta.data %>% 
  select(seurat_clusters, id) %>% 
  unique() %>% 
  add_count(seurat_clusters) %>% 
  arrange(n)
```

#### Lymphocytes
```{r}
lymphocytes <- lymphocytes %>% recluster(dims = dims)
```

```{r}
p <- seurat_feature(lymphocytes, facet_hide = TRUE)
p
```
```{r}
cluster_sample_tally <- lymphocytes@meta.data %>% 
  select(seurat_clusters, id) %>% 
  unique() %>% 
  add_count(seurat_clusters) %>% 
  arrange(n)

cluster_sample_tally
```
```{r}
clusters_to_remove <- cluster_sample_tally %>% filter(n == 1) %>% pull(seurat_clusters) %>% as.character()
clusters_to_remove
```

### Identify and remove B cells from clustering
```{r}
seurat_feature(lymphocytes, features = c('JCHAIN', 'IGKC'))
```
```{r}
VlnPlot(lymphocytes, 'IGKC')
```

#### Outlier cluster likely B lymphocytes
```{r}
outlier <- RunPresto(lymphocytes, ident.1 = 20) %>% rownames_to_column('gene')
outlier
```

### Extract T lymphocytes and recluster
```{r}
tcell_clusters <- lymphocytes$seurat_clusters %>% setdiff(c(20, clusters_to_remove))
tlymphocytes <- subset(lymphocytes, subset = (seurat_clusters %in% tcell_clusters)) %>% FindVariableFeatures(nfeatures = 3000) %>% ScaleData() %>% RunPCA() %>% RunHarmony(batch) 

ElbowPlot(tlymphocytes, ndims = 50, reduction = 'harmony')
```
```{r}
tlymphocytes <- tlymphocytes %>% recluster(dims = dims)

p <- seurat_feature(tlymphocytes, facet_hide = TRUE)
p
```

### Recombine and recluster
```{r}
combined_features <- union(apcs@assays$RNA@var.features, tlymphocytes@assays$RNA@var.features)
combined_features %>% length()

subcluster <- subset(seuratobj, 
                     subset = seurat_clusters %in% c(apc_clusters, tcell_clusters))

VariableFeatures(subcluster) <- combined_features

subcluster <- subcluster %>% 
  ScaleData() %>% 
  RunPCA() %>% 
  RunHarmony(group.by.vars = c('treatment', 'chemistry', 'patient', 'flow', 'mouse'))

```
```{r}
ElbowPlot(subcluster, ndims = 50, reduction = 'harmony')
```

### RunUMAP, FindNeighbors, and FindClusters 
```{r}
subcluster <- subcluster %>% recluster(dims = 1:20,
                                       resolution = c(0.2, 1, 1.6, 2, 2.4),
                                       default_ident = 'RNA_snn_res.2.4')

subcluster %>% seurat_feature(facet_hide = TRUE)
```


### Any clusters with only 1 patient?
```{r}
subcluster@meta.data %>% 
  select(seurat_clusters, id) %>% 
  unique() %>% 
  add_count(seurat_clusters) %>% 
  arrange(n)
```

### Add clusters from iterative subclustering 

```{r}
apc_clustering <- apc@meta.data %>% 
  rownames_to_column() %>% 
  select(rowname, seurat_clusters) %>% 
  mutate(seurat_clusters = paste0('APC_', seurat_clusters))

tlymphocyte_clustering <- tlymphocyte@meta.data %>% 
  rownames_to_column() %>% 
  select(rowname, seurat_clusters) %>% 
  mutate(seurat_clusters = paste0('T_', seurat_clusters))
```


## Export
```{r}
subcluster %>% write_rds(file.path(data_dir, 'seuratobj_subcluster.rds'))
```

```{r}
seuratobj@meta.data %>% select(clust = RNA_snn_res.2, id) %>% unique() %>% add_count(clust) %>% arrange(n)
```




## Session Info
```{r}
sessionInfo()
```



