...
counts_table <- FetchData(seuratobj,
                          vars = c('id', 'sample', 'ident', 'nCount_Spatial', paste0('spatial_', c(gene_set, genes_of_interest, genes_keratinocyte, genes_il17, genes_th1th2)) %>% unique), 
                          slot = 'counts') %>% 
  rownames_to_column() %>% 
  as_tibble() %>% 
...
p <- total_long_adjacent %>% 
  left_join(patient_metadata %>% select(id, category = group)) %>% 
  filter(gene %in% setdiff(genes_keratinocyte, 'CXCL1')) %>% 
  filter(cluster %in% c('Fibroblast')) %>% 
  group_by(group, treatment, gene) %>% 
  mutate(label = factor(paste(group, treatment),
                        levels = c(
                          'Responder Pre',
...
ggsave(plot = p,
       ...
       h = 5,
       w = 18,
       filename = paste0('keratinocyte_genes_pct_adjacent_stromal', distance, '.pdf'))
p
```
