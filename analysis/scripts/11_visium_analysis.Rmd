...
counts_table <- FetchData(seuratobj,
                          vars = c('id', 'sample', 'ident', paste0('spatial_', c(gene_set, genes_of_interest, genes_keratinocyte, genes_il17)) %>% unique), 
                          slot = 'counts') %>% 
  rownames_to_column() %>% 
  as_tibble() %>% 
...
p <- map(counts_distance_summarized$gene %>% unique(), function(i) {
