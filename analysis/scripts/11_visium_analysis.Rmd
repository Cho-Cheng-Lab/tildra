...
genes_th2 <- c('CXCL1', 'OASL', 'KRT16', 'CLDN8', 'TSLP', 'IL25', 'IL33', 'IL1RL1')
...
counts_table <- FetchData(seuratobj,
                          vars = c('id', 'sample', 'ident', 'nCount_Spatial', paste0('spatial_', c(gene_set, genes_of_interest, genes_keratinocyte, genes_il17, genes_th1th2, genes_stromal)) %>% unique), 
                          slot = 'counts') %>% 
  rownames_to_column() %>% 
  as_tibble() %>% 
...
p <- total_long_adjacent %>% 
  left_join(patient_metadata %>% select(id, category = group)) %>% 
  filter(gene %in% setdiff(genes_keratinocyte, 'CXCL1')) %>% 
  filter(cluster %in% c('Keratinocyte')) %>% 
  ggplot(aes(x = sample,
             y = counts,
  ...
  scale_fill_manual(values = rev(colors_response2)) +
...
p <- total_long_adjacent %>% 
  ...
  mutate(label = factor(paste(group, treatment),
                        levels = c(
                          'Responder Pre',
                          'Responder Mid',
                          'Non-responder Pre',
...
ggsave(plot = p,
       ...
       h = 6,
       w = 10,
       filename = paste0('keratinocyte_genes_adjacent_avg', distance, '.pdf'))
p
```
```{r}
p <- total_long_adjacent %>% 
  left_join(patient_metadata %>% select(id, category = group)) %>% 
  filter(gene %in% genes_stromal) %>% 
  filter(cluster %in% c('Fibroblast', 'Stromal')) %>% 
  group_by(group, treatment, gene) %>% 
  mutate(label = factor(paste(group, treatment),
                        levels = c(
                          'Responder Pre',
...
ggsave(plot = p,
       ...
       h = 6,
       w = 10,
       filename = paste0('stromal_genes_adjacent_avg', distance, '.pdf'))
p
```
...
ggsave(plot = p,
       ...
       h = 6,
       w = 10,
       filename = paste0('keratinocyte_genes_adjacent_mid', distance, '.pdf'))
p
```
...
ggsave(plot = p,
       ...
       h = 5,
       w = 8,
       filename = paste0('keratinocyte_genes_adjacent', distance, '_mean_by_gene.pdf'))
p
```
...
p <- total_long_adjacent %>% 
  filter(gene %in% genes_stromal) %>% 
  filter(cluster %in% c('Stromal', 'Fibroblast')) %>% 
  ggplot(aes(x = sample,
             y = pct_spots,
             fill = group)) +
  geom_bar(stat = 'summary',
...
ggsave(plot = p,
       ...
       h = 5,
       w = 18,
       filename = paste0('stromal_genes_pct_adjacent_stromal', distance, '.pdf'))
p
```
...
ggsave(plot = p,
       ...
       h = 8,
       w = 12,
       filename = 'distance_th1.pdf')

p
```
#### Th1 and Th2 genes
```{r}
gene_subset <- genes_th2
...
ggsave(plot = p,
       ...
       h = 8,
       w = 12,
       filename = 'distance_th2.pdf')

p
```
```{r}
gene_subset <- genes_keratinocyte
...
ggsave(plot = p,
       ...
       h = 8,
       w = 12,
       filename = 'distance_keratinocyte_il17.pdf')
p
```
...
```{r}
counts_distance_summarized %>% 
  write_tsv(here(output_dir, 'counts_by_distance.tsv'))
