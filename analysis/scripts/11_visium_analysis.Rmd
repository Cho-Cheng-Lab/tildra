...
counts_table <- FetchData(seuratobj,
                          vars = c('id', 'sample', 'ident', paste0('spatial_', c(gene_set, genes_of_interest, genes_keratinocyte, genes_il17, genes_control)) %>% unique), 
                          slot = 'counts') %>% 
  rownames_to_column() %>% 
  as_tibble() %>% 
...
```{r}
p <- total_long_adjacent %>% 
  filter(gene %in% c('IL17RA', 'IL17RC')) %>% 
  filter(cluster == 'Keratinocyte') %>% 
  ggplot(aes(x = sample,
             y = pct_spots,
             fill = group)) +
...
```{r}
group1 <- stat_input %>% filter(group == 'Responder') %>% pull(counts)
group2 <- stat_input %>% filter(group == 'Non-responder') %>% pull(counts)
...
```{r}
lymphoid_distance_to_spots <- lymphoid_spots %>% 
  map(function(i) {
    spatial_neighbors(seuratobj = seuratobj,
                      distance = 5,
                      barcode = i) 
...
counts_distance <- counts_table %>% 
  left_join(lymphoid_distance_to_spots %>% 
...
#p <- map(counts_distance_summarized$gene %>% unique() %>% intersect(c(genes_keratinocyte)), function(i) {
...
distance_cor <- map(genes_to_check, function(i) {
  ...
  counts_table <- FetchData(seuratobj,
                          vars = c('id', 'sample', 'ident', paste0('spatial_', i) %>% unique), 
                          slot = 'counts') %>% 
  rownames_to_column() %>% 
  as_tibble()
