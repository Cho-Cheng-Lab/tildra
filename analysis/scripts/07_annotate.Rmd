...
##### SciBet or Seurat
```{r}
labels1 <- read_tsv(here('analysis/output/06_label/labels.tsv.gz')) %>% select(rowname, scibet = label)
labels2 <- read_tsv(here('analysis/output/06_label/transfer.tsv.gz')) %>% select(rowname, seurat = predicted.id)

label_transfer <- metadata %>% select(rowname, local, global) %>% left_join(labels1) %>% left_join(labels2)
label_transfer %>% head()
```
#### Which label to use?
```{r}
...
label_match <- label_odds %>% 
  pivot_wider(names_from = 'label',
              values_from = 'group2') %>% 
  mutate(match = ifelse(scibet == seurat, 'Yes', 'No'))
...
merged_annotations <- metadata %>% 
  ...
  left_join(label_transfer %>% select(rowname, label = scibet)) %>% 
