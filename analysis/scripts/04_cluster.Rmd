...
seuratobj <- seuratobj %>% 
  FindNeighbors(reduction = 'harmony', dims = dims, k.param = neighbors, annoy.metric = metric) %>% 
  FindClusters(resolution = c(0.1, 1), prune.SNN = prune) %>% 
  RunUMAP(reduction = 'harmony', dims = dims, n.neighbors = neighbors, metric = metric) 
```
