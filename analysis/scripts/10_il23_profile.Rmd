...
Group-level
```{r}
de_pre_vs_mid_response <- bind_rows(read_tsv(here('analysis/output/08_de', which_cluster, 'de_pre_vs_mid_responder.tsv.gz')) %>% mutate(response = 'Responder'),
                                    read_tsv(here('analysis/output/08_de', which_cluster, 'de_pre_vs_mid_nonresponder.tsv.gz')) %>% mutate(response = 'Non-responder'))
...
```{r}
de_responder_vs_nonresponder <- read_tsv(here('analysis/output/08_de', which_cluster, 'de_responder_vs_nonresponder_mid.tsv.gz'))
```
...
```{r eval=FALSE}
de_pre_vs_mid_apc <- read_tsv(here('analysis/output/08_de', which_cluster, 'de_pre_vs_mid.tsv.gz'))
