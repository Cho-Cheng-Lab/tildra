...
patient_meta <- seurat_meta %>% 
  select(id, patient, sample, condition, treatment, response, response_category) %>% 
  unique() %>% 
  as_tibble()
...
```{r}
patient_display <- seurat_meta %>% select(id, patient, condition, treatment, response, response_category) %>% unique() %>% 
  filter(treatment != 'None') %>% 
  pivot_wider(names_from = treatment,
              values_from = id) %>%
  mutate(display = paste0(patient, ' (', Pre, ', ', Mid, '): ', response_category))
...
meta <- seurat_meta %>% 
  ...
  select(rowname, id, patient, sample, condition, treatment, response, response_category, merged_label) %>% 
  mutate(display = paste(patient, response_category),
...
```{r}
de_pre_vs_post_apc <- read_tsv('analysis/output/07_de/de_pre_vs_post_response_apc.tsv.gz')

apc_responders <- de_pre_vs_post_apc %>% 
  filter(response == 'responders',
         p_val_adj < 0.05)
...
```{r}
apc_nonresponders <- de_pre_vs_post_apc %>% 
...
```{r}
data_table <- 
  meta %>% select(rowname, id, sample, patient, condition, treatment, response, response_category, display, group) %>% 
...
data_table_long_tx_summarized <- 
  ...
  group_by(id, sample, patient, condition, group, display, treatment, response, response_category, gene) %>% 
...
tx_patients_responders <- meta %>% filter(treatment != 'None', response_category == 'Responder') %>% pull(patient) %>% unique() %>% sort() %>% set_names(.) 

tx_patients_nonresponders <- meta %>% filter(treatment != 'None', response_category == 'Nonresponder') %>% pull(patient) %>% unique() %>% sort() %>% set_names(.) 
...
radar_plots <- map(plot_input$patient %>% unique %>% sort, function(i) {
  ...
  response <- plot_input %>% filter(patient == which_patient) %>% pull(response_category) %>% unique()
...
p_waterfall <- map(tx_patients, function(i) {
  ...
  labs(x = '',
       ...
       title = paste0('Patient ', i, ': IL23 Response with Tildra'))
...
perfect_separators <- data_table_long_tx_summarized %>% 
  filter(gene %in% nonresponder_genes,
         treatment == 'Mid',
         group == 'Trm') %>% 
  select(group, response_category, gene, expression) %>% 
  group_by(response_category, group, gene) %>% 
...
p <- data_table_long_tx_summarized %>% 
  ...
  filter(gene %in% c(il23_genes, 'IL17A', 'IL17F'),
         treatment == 'Mid',
         group == 'Trm') %>% 
  ggplot(aes(x = response_category,
             y = expression,
             color = response_category)) +
  #geom_boxplot(outlier.color = NA) +
  geom_bar(stat = 'summary', position = 'dodge', fill = 'grey90') +
...
p <- data_table_long %>% 
  filter(treatment != 'None',
         ...
         group %in% c('APC', 'Trm', 'Tcell')) %>% 
  group_by(patient, treatment, response_category, group, gene) %>% 
...
meta %>% 
  filter(merged_label %in% clusters_apc,
         treatment != 'None') %>% 
  group_by(response_category, merged_label, treatment) %>% 
  tally() %>% 
  pivot_wider(names_from = c(response_category, treatment),
              values_from = n) %>% 
  View()
