...
visium_meta <- read_tsv(here('analysis/output/10_visium/visium_meta.tsv.gz')) %>% 
  mutate(id = ifelse(id == 'S234', 'S277', id), cluster = annotation)
...
clusters_t <- metadata %>% filter(str_detect(cluster, 'T')) %>% pull(cluster) %>% unique()
clusters_apc <- metadata %>% filter(celltype == 'APC') %>% pull(cluster) %>% unique()
```
...
```{r}
meta <- metadata %>% 
  select(rowname, id, cluster) %>% 
  bind_rows(visium_meta %>% select(rowname, id, cluster) %>% drop_na()) %>% 
  left_join(patient_meta) %>% 
  select(rowname, id, patient, sample, condition, treatment, response, group, cluster) %>% 
  mutate(display = paste(patient, group),
         ...
         cellgroup = case_when(
           ...
           cluster %in% clusters_apc ~ 'APC',
           cluster %in% clusters_t ~ 'Tcell',
           TRUE ~ cluster
         ))
...
Group-level
```{r}
de_pre_vs_mid_response <- bind_rows(read_tsv(here('analysis/output/07_de', which_cluster, 'de_pre_vs_mid_responder.tsv.gz')) %>% mutate(response = 'Responder'),
                                    read_tsv(here('analysis/output/07_de', which_cluster, 'de_pre_vs_mid_nonresponder.tsv.gz')) %>% mutate(response = 'Non-responder'))
...
Patient-level
```{r}
de_pre_vs_mid_patients <- read_tsv(here('analysis/output/07_de', which_cluster,'de_pre_vs_mid_per_patient.tsv.gz'))
...
```{r}
il23_genes <- il23_response %>% filter(str_detect(cluster, 'Trm1'), p_val_adj < 0.05, n_responders > n_nonresponders) %>% pull(gene)
il23_genes_strict <- il23_response %>% filter(str_detect(cluster, 'Trm1'), p_val_adj < 0.05, n_responders > 2, n_responders > n_nonresponders, n_nonresponders < 2) %>% pull(gene) %>% unique()
...
```{r}
de_responder_vs_nonresponder <- read_tsv(here('analysis/output/07_de', which_cluster, 'de_responder_vs_nonresponder_mid.tsv.gz'))
```
...
```
```{r}
nonresponder_genes <- nonresponder_markers %>% filter(str_detect(cluster, 'Trm1')) %>% pull(gene)
```

### Tildra response in APCs
```{r eval=FALSE}
de_pre_vs_mid_apc <- read_tsv(here('analysis/output/07_de', which_cluster, 'de_pre_vs_mid.tsv.gz'))
...
barcodes_visium <- visium_meta %>% 
  filter(cluster %in% c('fibroblast', 'keratinocyte')) %>% 
  pull(rowname)
...
```{r}
de_pre_vs_mid_response %>% filter(response == 'Responder', str_detect(cluster, 'Trm1'), p_val_adj < 0.01, avg_log2FC > 0) %>% pull(gene) %>% intersect(il23_genes)
```
