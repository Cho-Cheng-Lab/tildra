...
tx_patients_nonresponders <- meta %>% filter(treatment != 'None', response_group == 'Nonresponder') %>% pull(patient) %>% unique() %>% sort() %>% set_names(.) 
...
group2 <- data_subset %>% filter(treatment == 'Post') %>% pull(expression)
