...
```{r}
il23_genes <- il23_response %>% filter(str_detect(cluster, 'Trm'), p_val_adj < 0.05) %>% pull(gene)
il23_genes_strict <- il23_response %>% filter(str_detect(cluster, 'Trm'), p_val_adj < 0.05, n_responders > 2) %>% pull(gene) %>% unique()
