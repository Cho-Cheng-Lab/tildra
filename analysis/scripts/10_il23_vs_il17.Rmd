...
aggregate_matrix <- function(input_matrix, 
  ...
  if(!is.null(rows)) {
    ...
    rownames(input_matrix) <- rows
    
    aggregated_matrix <- t(sapply(by(input_matrix, rownames(input_matrix), colSums), identity))
  }
  
  if(!is.null(cols)) {
    
    input_matrix <- t(input_matrix)
    rownames(input_matrix) <- cols
    aggregated_matrix <- sapply(by(input_matrix, rownames(input_matrix), colSums), identity)
  }
