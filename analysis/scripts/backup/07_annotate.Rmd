...
merged_annotations <- metadata %>% 
  ...
  left_join(rashx_clusters %>% select(barcode, order, cluster, supercluster)) %>% 
  as_tibble()
...
### Agreement metrics
```{r}
all_clusterings <- c('local', 'global', 'label', 'cluster', 'supercluster')
...
annotations <- c('label', 'cluster', 'supercluster') %>% set_names(.)
...
map(merged_annotations$local %>% unique() %>% sort(), function(i) {
  ...
  ggsave(plot = p,
    ...
    # 2-way: local vs rashx and label
  p <- plot_sankey2(merged_annotations %>% filter(local == i),
                    nodes = c('cluster', 'local', 'label'),
                    names = c('RashX Cluster', 'Local Cluster', 'Label Transfer'))
  
  ggsave(plot = p,
         filename = paste0(i, '.png'),
         path = here(output_dir, 'local', 'sankey2_local_vs_rashx_label'),
         dpi = dpi,
         h = 4,
         w = 8)
...
map(merged_annotations$global %>% unique() %>% sort(), function(i) {
  ...
  # 2-way: global vs rashx and label
  p <- plot_sankey2(merged_annotations %>% filter(local == i),
                    nodes = c('cluster', 'global', 'label'),
                    names = c('RashX Cluster', 'Global Cluster', 'Label Transfer'))
  
  ggsave(plot = p,
         filename = paste0(str_remove(i, '/'), '.png'),
         path = here(output_dir, 'global', 'sankey2_local_vs_rashx_label'),
         dpi = dpi,
         h = 4,
         w = 8)
