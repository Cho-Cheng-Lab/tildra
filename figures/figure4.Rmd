...
```{r}
genes_of_interest <- c('IL17F', 'IL10', 'IFNG')
...
eclipse_tallies <- eclipse %>% 
  mutate(IL17F = as.numeric(IL17F > 0),
         IL10 = as.numeric(IL10 > 0),
         IFNG = as.numeric(IFNG > 0)) %>% 
  select(patient, treatment, IL17F, IL10, IFNG) 
...
eclipse_il17f <- eclipse_tallies %>% 
  ...
  group_by(patient, treatment, n, IL17F) %>% 
  tally() %>% 
  mutate(pct = nn/n * 100) %>% 
  filter(IL17F > 0)
...
```{r}
p <- eclipse_grouped %>% 
  mutate(gene = factor(gene, levels = c('IL17F', 'IL10', 'IFNG'))) %>% 
  ggplot(aes(x = gene, 
             y = pct, 
             fill = treatment)) +
...
```{r}
# ray th1 th2 genes
genes_th1 <- c('CXCL9', 'CXCL10', 'CXCL11', 'ICAM1')
