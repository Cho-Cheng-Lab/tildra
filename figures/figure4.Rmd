...
facet_input <- FetchData(seuratobj,
  ...
  #mutate(group = ifelse(patient == 'P9', 'T Cell Responder', as.character(group))) %>% 
  #mutate(group = factor(group, levels = c('Responder', 'T Cell Responder', 'Non-responder'))) %>% 
...
```{r}
which_gene <- 'IL17A'
tallies <- facet_input %>% group_by(category) %>% tally() %>% pull(n) %>% min()
...
p <- map(cytokines, function(which_gene) {
  ...
  total_cpm <- nonzero_input %>% group_by(category, treatment) %>% summarize(total = round(sum(10^expression), 2))
  ...
  p <- nonzero_input %>% 
    ...
    rasterize(geom_point(alpha = 0.5,
               size = 1), dpi = 300) +
    facet_wrap(~category + treatment, nrow = 1) +
    labs(x = 'UMAP1',
         y = 'UMAP2',
         title = which_gene) +
