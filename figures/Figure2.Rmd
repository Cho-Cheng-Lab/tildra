...
p <- map(plot_clusters, function(which_cluster) {
  ...
  heatmap_subset <- heatmap_data %>% 
    ...
    filter(PV > 0) %>% 
    slice_max(PV, n = 500) %>% 
    select(gene, PV, Mid, AD) %>% 
    pivot_longer(cols = -gene,
                 names_to = 'condition',
                 values_to = 'value') %>% 
    arrange(value)
  
  cor_subset <-  heatmap_subset %>% 
  ...
  average_l2fc <- cor_subset$Mid %>% abs() %>% mean()
  n_deg <- de_tally %>% filter(cluster == which_cluster) %>% pull(n)
  ...
  heatmap_subset %>% 
    ...
    labs(x = '',
         y = '',
         title = which_cluster,
         subtitle = paste('R = ', round(pv_mid_cor, 3), 
                          '\n DEGs: ', n_deg, 
                          '\n Avg log2FC: ', round(average_l2fc, 3))) +
...
ggsave(plot = p,
       ...
       w = 8,
       h = 8,
       filename = 'ultracluster_500_fc.pdf')
```
