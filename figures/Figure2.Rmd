...
de_combined <- read_tsv(here('analysis/output/07_de/', which_annotation, 'de_combined.tsv.gz'))
```
...
```{r}
analyze <- metadata %>% 
  select(rowname, id, sample, patient, condition, treatment, cluster, sample_label, annotation, group) %>% 
  mutate(description = factor(paste(condition, treatment) %>% str_remove(' None'),
                              levels = c('PV Pre',
                                         ...
                                         'AD',
                                         'HC')),
         sample_label = paste0(sample, ' (', id, '), ', group)) %>% 
...
```{r}
tallies <- analyze %>% 
  group_by(patient, condition, treatment, annotation) %>% 
  tally()
...
cluster_levels <- seuratobj$annotation
...
de_subset <- de_combined %>% 
  ...
  mutate(comparison = factor(comparison, levels = which_comparison),
         ...
         cluster = factor(cluster, levels = levels(seuratobj$annotation)))
```
...
```{r}
merged_table <- exprs %>% 
  inner_join(metadata_subset %>% select(rowname, patient, id, condition, treatment, response, group, annotation)) %>% 
...
summarized_table <- merged_table %>% 
  group_by(treatment, group, annotation, gene) %>% 
...
```{r}
merged_table <- exprs %>% 
  inner_join(metadata_subset %>% select(rowname, patient, id, condition, treatment, group, annotation, patient_label)) %>% 
  pivot_longer(cols = genes_to_extract,
               names_to = 'gene',
               values_to = 'expression')
...
plot_input <- merged_table %>% 
  group_by(patient, treatment, group, annotation, gene, patient_label) %>% 
  summarize(mean = mean(expression)) 
...
```{r}
summarized_table <- merged_table %>% 
  group_by(patient, treatment, group, annotation, gene) %>% 
