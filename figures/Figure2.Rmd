...
#### Global
```{r}
heatmap_cells <- seuratobj@meta.data %>% 
  #filter(condition != 'AD') %>% 
  rownames()
...
#plot_clusters <- c('Mac', 'LC', 'Mast', 'Trm', 'CTL')
...
cor_metrics <-  map(l2fc$ultracluster, function(i) {
...
p1 <- map(l2fc$ultracluster, function(which_cluster) {
...
p2 <- l2fc %>% 
  ggplot(aes(x = reorder(ultracluster, -l2fc),
             y = l2fc)) +
  geom_point(size = 2,
             color = 'darkorchid4') +
  labs(x = '',
...
p3 <- cor_metrics %>% 
  pivot_longer(cols = -ultracluster,
               names_to = 'group',
               values_to = 'cor') %>% 
  ggplot(aes(x = factor(ultracluster, levels = l2fc$ultracluster),
             y = cor,
             color = group)) +
  geom_point(size = 2) +
  labs(x = '',
...
p <- patchwork::wrap_plots(p1, p2, p3, nrow = 3, heights = c(8, 1, 1))
...
ggsave(plot = p,
       ...
       w = 12,
       h = 8,
       filename = 'ultracluster_100_figure.pdf')
```
...
heatmap_data %>% arrange(gene, ultracluster)
```
```{r}
tic()
heatmap_data <- pseudobulk2(seuratobj,
                            cells = cells,
                            genes = genes,
                            group_by = group_by)
toc()
