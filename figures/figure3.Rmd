...
```
```{r}
clusters <- plot_input$cluster %>% unique()
p_boxplots <- map(clusters, function(which_cluster) {
  ...
  if(which_cluster == clusters[ceiling(length(clusters)/2)]) {
...
### Boxplots
```{r}
map(plot_input$cluster %>% unique(), function(which_cluster) {
  ...
  plot_input %>% 
    ...
    mutate(description = factor(description, levels = c('HC', 'PV', 'Mid'))) %>% 
    ggplot(aes(x = gene,
               y = expression,
               fill = description)) +
    geom_boxplot() +
