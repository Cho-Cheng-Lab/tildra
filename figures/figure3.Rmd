...
p <- logfc_input %>% 
  filter(cluster == which_subset) %>% 
  mutate(avg_log2FC = ifelse(p_val_adj > p_adj_cutoff, N, avg_log2FC)) %>% 
...
patient_level_fc <- pseudo_data %>% 
  filter(treatment != 'None') %>% 
  inner_join(top_genes %>% group_by(cluster) %>% slice_max(avg_log2FC, n = 5) %>% select(supercluster = cluster, gene)) %>% 
