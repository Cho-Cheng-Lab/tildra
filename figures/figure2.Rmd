...
pseudo_data <- pseudobulk(seuratobj,
                          genes = union(top_genes_sig$gene, top_genes_nonsig$gene),
...
patient_level_fc <- pseudo_data %>% 
  filter(treatment != 'None') %>% 
  inner_join(bind_rows(top_genes_sig, top_genes_nonsig) %>% group_by(cluster) %>% select(supercluster = cluster, gene)) %>% 
