...
ggsave(plot = p[[2]] + p[[15]] + p[[9]] + p[[1]] + p[[14]] + p[[10]],
       device = cairo_pdf,
       path = output_dir,
       h = 7,
       w = 4.5,
...
p <- tally_input %>% 
  ...
  geom_col(color = 'black',
           position = 'dodge') +
  theme(legend.position = 'top') +
  scale_fill_few(palette = 1, drop = FALSE) +
...
pseudo_data <- pseudobulk(seuratobj,
                          genes = c(top_genes_sig$gene, top_genes_nonsig$gene, pv50_genes) %>% unique(),
...
```{r}
which_subset <- 'Trm'
which_genes <- c('IL17A', 'IL26', 'GNLY', 'GZMB', 'CPM', 'SLA2', 'SOX4',
                 'IL2RA', 'IL1RL1', 'KDZF4', 'TNFRSF4', 'ZEB2', 'AREG', 'S100A8')
...
p <- boxplot_input %>% 
  ...
  scale_color_tableau() +
  theme(strip.text.x = element_text(face = 'italic')) +
  scale_y_continuous(breaks = c(0, 0.7, 1, 1.7, 2, 3),
                     labels = c(0, 5, 10, 50, 100, 1000)) +
  labs(y = 'Counts per Million',
       x = NULL)
