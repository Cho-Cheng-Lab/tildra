...
```{r}
p1 <- seurat_feature(patient_only, features = 'patient', label = FALSE, facet_hide = TRUE)
...
```{r}
p2 <- seurat_feature(patient_only, features = 'patient', facets = 'patient', label = FALSE, facet_hide = TRUE, alpha = 0.5, size = 0.2)
...
```{r}
plot_input <- analyze %>% 
  filter(description %in% c('PV Pre', 'PV Mid', 'HC')) %>% 
...
plots <- map(1:3, function(i) {
  ...
  plot_input %>% 
    filter(description != 'HC') %>% 
    group_by(patient, treatment, celltype, group) %>% 
    summarize(n = sum(n)) %>% 
