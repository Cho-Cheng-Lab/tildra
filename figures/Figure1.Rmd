...
seuratobj$treatment <- factor(seuratobj$treatment, levels = c('None', 'Pre', 'Post'))
```
...
plot_input <- analyze %>% 
  ...
  group_by(sample, patient, description, cluster, merged_label, treatment, response_category) %>% 
  tally() %>% 
  group_by(sample) %>% 
  mutate(total = sum(n),
         fraction = n/total)
...
```
```{r}
patient_info <- plot_input %>% select(sample, patient, description, response_category) %>% unique() %>% arrange(patient, description)
text_color <- ifelse(patient_info$response_category == 'Responder', 'dodgerblue4', 'orangered')
```
