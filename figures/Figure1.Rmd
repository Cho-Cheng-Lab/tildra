...
p_celltypes <- map(1:length(conditions), function(i) {
...
trm_clusters <- str_subset(cluster_tallies$cluster, 'Trm') %>% unique() %>% sort()

p_trm <- map(1:length(trm_clusters), function(i) {
  
  p <- cluster_tallies %>% 
    filter(treatment %in% c('Pre', 'Mid')) %>% 
    filter(cluster == trm_clusters[i]) %>% 
    ggplot(aes(x = treatment,
               y = n,
               fill = cluster)) +
    geom_boxplot() +
...
```{r}
p_celltype <- map(1:length(conditions), function(i) {
...
ggsave(plot = p,
       device = cairo_pdf,
       filename = 'F1E_box_cluster_n.pdf',
       path = output_dir,
       h = 8,
       w = 8)
...
ggsave(plot = p,
       device = cairo_pdf,
       filename = 'F1E_box_cluster_f.pdf',
       path = output_dir,
       h = 8,
       w = 8)
